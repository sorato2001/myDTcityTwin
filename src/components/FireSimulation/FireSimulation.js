...
export class FireEffect {
  ...
  init() {
    ...
    let particleSystem = this.scene.primitives.add(
      new Cesium.ParticleSystem({
        image: 'data/model/fire.png', //生成所需粒子的图片路径
        //粒子在生命周期开始时的颜色
        ...
        sizeInMeters: true,
        //系统的粒子发射器
        emitter: new Cesium.CircleEmitter(Cesium.Math.toRadians(5.0)), // BoxEmitter 盒形发射器， ConeEmitter 锥形发射器， SphereEmitter 球形发射器， CircleEmitter 圆形发射器
      })
    );
  ...
  remove() {
    this.removeEvent(); // Call the removeEvent function
